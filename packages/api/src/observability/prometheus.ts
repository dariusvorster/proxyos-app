...
export async function renderPrometheusMetrics(db: Db): Promise<string> {
  ...
  const [routeRows, metricRows, agentRows, certRows, connRows] = await Promise.all([
  ...
  for (const r of metricRows) {
    const cur = byRoute.get(r.routeId) ?? { requests: 0, status2xx: 0, status4xx: 0, status5xx: 0, latencySum: 0, bytes: 0 }
  ...
  lines.push('# HELP proxyos_route_bytes_total Total bytes transferred per route (5-min window)')

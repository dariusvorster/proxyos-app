...
export function buildCaddyRoute(route: Route, opts: BuildOptions = {}): CaddyRoute {
  ...
  //   at the server level (see buildTrustedProxies). Do not set them here.
    ...
    headers: {
      request: {
        set: {
          ...
          // X-Forwarded-* deliberately NOT set here.
          // Caddy's reverse_proxy handler manages them natively when
          // trusted_proxies is configured at the server level.
        },
      },
    },
    ...transportBlock,
...
// TODO: make trusted_proxies configurable via env var or settings
// TODO: auto-refresh Cloudflare ranges from cloudflare.com/ips-{v4,v6}
export function buildTrustedProxies(): TrustedProxiesConfig {
  return {
    source: 'static',
    ranges: [
      // Cloudflare IP ranges (IPv4) — https://www.cloudflare.com/ips/
      '173.245.48.0/20',
      '103.21.244.0/22',
      '103.22.200.0/22',
      '103.31.4.0/22',

...
export default function RouteDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const rulesQuery = trpc.routeRules.listByRoute.useQuery({ routeId: id })
  const createRuleMut = trpc.routeRules.create.useMutation({ onSuccess: () => { rulesQuery.refetch(); setRuleForm(null) } })
  const deleteRuleMut = trpc.routeRules.delete.useMutation({ onSuccess: () => rulesQuery.refetch() })
  const [ruleForm, setRuleForm] = useState<{ matcherType: 'path' | 'header' | 'query' | 'method'; matcherKey: string; matcherValue: string; action: 'upstream' | 'redirect' | 'static'; upstream: string; redirectUrl: string; staticBody: string; staticStatus: string; priority: string } | null>(null)
  ...
  return (
    <>
      ...
      <PageContent>
        ...
        <Card header={
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span>Routing rules — {rulesQuery.data?.length ?? 0}</span>
            <Button size="sm" variant="ghost" onClick={() => setRuleForm({ matcherType: 'path', matcherKey: '', matcherValue: '', action: 'upstream', upstream: '', redirectUrl: '', staticBody: '', staticStatus: '200', priority: '0' })}>+ Add rule</Button>
          </div>
        }>
          {ruleMsg && <p style={{ fontSize: 11, color: ruleMsg.startsWith('Error') ? 'var(--red)' : 'var(--green)', margin: '0 0 8px' }}>{ruleMsg}</p>}
          {ruleForm && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginBottom: 10, padding: 10, background: 'var(--surface2)', borderRadius: 6, border: '1px solid var(--border)' }}>
              <div>
                <div style={{ fontSize: 10, color: 'var(--text2)', marginBottom: 3 }}>Match type</div>
                <select value={ruleForm.matcherType} onChange={e => setRuleForm(f => f && ({ ...f, matcherType: e.target.value as typeof f.matcherType }))}
                  style={{ width: '100%', padding: '4px 6px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 4, color: 'var(--text)', fontSize: 12 }}>
              ...
              {(ruleForm.matcherType === 'header' || ruleForm.matcherType === 'query') && (
                <div>
                  <div style={{ fontSize: 10, color: 'var(--text2)', marginBottom: 3 }}>Key</div>
                  <Input value={ruleForm.matcherKey} onChange={e => setRuleForm(f => f && ({ ...f, matcherKey: e.target.value }))} placeholder="Header / query key" style={{ fontSize: 12 }} />
                </div>
              )}
              <div>
                <div style={{ fontSize: 10, color: 'var(--text2)', marginBottom: 3 }}>Value</div>
                <Input value={ruleForm.matcherValue} onChange={e => setRuleForm(f => f && ({ ...f, matcherValue: e.target.value }))} placeholder="/api/* or GET" style={{ fontSize: 12 }} />
              </div>
              <div>
                <div style={{ fontSize: 10, color: 'var(--text2)', marginBottom: 3 }}>Action</div>
                <select value={ruleForm.action} onChange={e => setRuleForm(f => f && ({ ...f, action: e.target.value as typeof f.action }))}
                  style={{ width: '100%', padding: '4px 6px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 4, color: 'var(--text)', fontSize: 12 }}>
              ...
              {ruleForm.action === 'upstream' && (
                <div>
                  <div style={{ fontSize: 10, color: 'var(--text2)', marginBottom: 3 }}>Upstream</div>
                  <Input value={ruleForm.upstream} onChange={e => setRuleForm(f => f && ({ ...f, upstream: e.target.value }))} placeholder="10.0.0.5:8080" style={{ fontSize: 12 }} />
                </div>
              )}
              {ruleForm.action === 'redirect' && (
                <div>
                  <div style={{ fontSize: 10, color: 'var(--text2)', marginBottom: 3 }}>Redirect URL</div>
                  <Input value={ruleForm.redirectUrl} onChange={e => setRuleForm(f => f && ({ ...f, redirectUrl: e.target.value }))} placeholder="https://…" style={{ fontSize: 12 }} />
                </div>
              )}
              {ruleForm.action === 'static' && (
                <>
                  <div>
                    <div style={{ fontSize: 10, color: 'var(--text2)', marginBottom: 3 }}>Status</div>
                    <Input value={ruleForm.staticStatus} onChange={e => setRuleForm(f => f && ({ ...f, staticStatus: e.target.value }))} placeholder="200" style={{ fontSize: 12 }} />
                  </div>
                  <div>
                    <div style={{ fontSize: 10, color: 'var(--text2)', marginBottom: 3 }}>Body</div>
                    <Input value={ruleForm.staticBody} onChange={e => setRuleForm(f => f && ({ ...f, staticBody: e.target.value }))} placeholder="ok" style={{ fontSize: 12 }} />
                  </div>
                </>
              )}
              <div>
                <div style={{ fontSize: 10, color: 'var(--text2)', marginBottom: 3 }}>Priority</div>
                <Input value={ruleForm.priority} onChange={e => setRuleForm(f => f && ({ ...f, priority: e.target.value }))} placeholder="0" style={{ fontSize: 12 }} />
              </div>
              <div style={{ gridColumn: '1 / -1', display: 'flex', gap: 8 }}>
                <Button variant="primary" size="sm"
                  disabled={createRuleMut.isPending || !ruleForm.matcherValue}
                  onClick={() => {
                    createRuleMut.mutate({
                      routeId: id,
                      matcherType: ruleForm.matcherType,
                      matcherKey: ruleForm.matcherKey || null,
                      matcherValue: ruleForm.matcherValue,
                      action: ruleForm.action,
                      upstream: ruleForm.action === 'upstream' ? ruleForm.upstream || null : null,
                      redirectUrl: ruleForm.action === 'redirect' ? ruleForm.redirectUrl || null : null,
                      staticBody: ruleForm.action === 'static' ? ruleForm.staticBody || null : null,
                      staticStatus: ruleForm.action === 'static' ? Number(ruleForm.staticStatus) || 200 : null,
                      priority: Number(ruleForm.priority) || 0,
                    }, { onError: (e) => setRuleMsg(`Error: ${e.message}`) })
                  }}>
                  {createRuleMut.isPending ? 'Saving…' : 'Save rule'}
                </Button>
                <Button variant="ghost" size="sm" onClick={() => setRuleForm(null)}>Cancel</Button>
              </div>
            </div>
          )}
          {(!rulesQuery.data || rulesQuery.data.length === 0) && !ruleForm ? (
            <div style={{ padding: '12px 0', fontSize: 12, color: 'var(--text3)' }}>No routing rules. Rules override the default upstream for matching requests.</div>
          ) : (
            <DataTable>
              <thead>
                <tr>
                  <th style={{ ...th, width: '6%' }}>Priority</th>
                  <th style={{ ...th, width: '14%' }}>Match type</th>
                  <th style={{ ...th, width: '20%' }}>Value</th>
              ...
              <tbody>
                {rulesQuery.data?.map(r => (
                  <tr key={r.id}>
                    <td style={{ ...td, fontFamily: 'var(--font-mono)', fontSize: 11 }}>{r.priority}</td>
                    <td style={td}><Badge tone="neutral">{r.matcherType}{r.matcherKey ? `: ${r.matcherKey}` : ''}</Badge></td>
                    <td style={{ ...td, fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--pu-400)' }}>{r.matcherValue}</td>
                    <td style={td}><Badge tone={r.action === 'redirect' ? 'amber' : r.action === 'static' ? 'blue' : 'green'}>{r.action}</Badge></td>
                    <td style={{ ...td, fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text2)' }}>
                      {r.upstream ?? r.redirectUrl ?? `${r.staticStatus ?? 200} ${r.staticBody ?? ''}` ?? '—'}
                    </td>
                    <td style={td}>
                      <Button variant="ghost" style={{ fontSize: 11, color: 'var(--red)', padding: '2px 8px' }}
                        onClick={() => deleteRuleMut.mutate({ id: r.id }, { onError: (e) => setRuleMsg(`Error: ${e.message}`) })}>
                        Delete
                      </Button>
                    </td>
                  </tr>

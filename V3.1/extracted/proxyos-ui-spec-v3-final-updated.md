# ProxyOS — UI/UX Design Spec v3.1
...
### Logomark — Grid Pivot (locked, do not change)

The ProxyOS mark is the **grid pivot**: four quadrant squares arranged in a 2×2 grid, meeting at a bright white centre square. The four quadrants use graduating purple tones — the top-left is the brightest (primary accent), the centre pivot is pure white. Represents the proxy as the crossroads everything passes through. Scales perfectly from 512px down to 14px because the shape reads as a bold 2×2 grid at any size.

**Icon background:** always `#0D0D1A` (near-black). The purple lives in the quadrants, not the background.
...
At 16px and below (favicon, browser tab): the four quadrants render as four bold coloured squares meeting at a white point — maximally readable, no detail lost. At 26px+ (sidebar): full detail including the rounded corners on each quadrant and the precise pivot square. No simplified version needed — the mark is already optimised for small sizes.
...
`v3.1` — IBM Plex Mono 10px, background `var(--surf2)`, colour `var(--text3)`, border-radius 4px, padding 1px 5px.
...
**Three icon buttons — own row ABOVE the avatar+name row:**
```css
display: flex;
gap: 2px;
align-items: center;
margin-bottom: 6px;
/* left-aligned — NOT right-aligned, NOT inline with avatar */
```
Each button:
```css
width: 26px;
...
**Layout stack (top to bottom inside the user area):**
```
[⚙]  [☽]  [→]          ← icon buttons row (left-aligned, above avatar)
[D]  Darius Vorster      ← avatar circle + name inline
     self-hosted · v3    ← tier · version subline
```

The icon buttons are on their own row above the avatar. NOT to the right of the avatar. NOT inline with the name. This is the confirmed OS family standard.
...
*ProxyOS UI/UX Design Spec v3.1 — proxyos.app — Homelab OS Family Standard — April 2026*
